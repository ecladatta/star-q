...
import { deleteCorpus, getCorpus, getCorpusOwner, getMoveTargets, moveCorpusToTeam, updateCorpusArchived } from '@/actions/corpus/corpusActions'
...
export default async function CorpusSettingsPage({ params }: { params: Promise<{ corpusId: string }> }) {
  ...
  const archived = corpus.status === 'archived'
  ...
  return (
    <Page>
      ...
      <CorpusSettingsPanel
        ...
        canEdit={canEditCorpus(access, corpus.status)}
              ...
              <div className="flex items-center justify-between gap-4 p-4">
                <div className="space-y-1">
                  <p className="text-sm font-medium text-foreground">
                    {archived ? 'Unarchive this corpus' : 'Archive this corpus'}
                  </p>
                  <p className="text-xs text-muted-foreground">
                    {archived
                      ? 'This restores editing for everyone who has access to the corpus.'
                      : 'This makes the corpus read-only for everyone. You can unarchive it at any time.'}
                  </p>
                </div>
                <ConfirmActionButton
                  action={async () => {
                    'use server'
                    await updateCorpusArchived(corpusId, !archived)
                  }}
                  title={archived ? 'Unarchive this corpus?' : 'Archive this corpus?'}
                  description={archived
                    ? (
                        <>
                          Editing will be restored for
                          {' '}
                    ...
                    : (
                        <>
                          ...
                          <strong>{corpus.title}</strong>
                          {' '}
                          read-only for everyone. You can unarchive it at any time.
                        </>
                      )}
                  confirmLabel={archived ? 'Unarchive corpus' : 'Archive corpus'}
                  successMessage={archived ? 'Corpus unarchived' : 'Corpus archived'}
                >
