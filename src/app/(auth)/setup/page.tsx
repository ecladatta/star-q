    ...
    <main className="mx-auto w-full max-w-md px-4 py-10">
      <div className="space-y-6 rounded-xl border border-border bg-card p-6 sm:p-8">
        <div className="space-y-2 text-center">
          <h1 className="text-lg font-semibold tracking-[-0.01em]">Set up STAR-Q</h1>
        ...
        {session?.user?.valid
          ? (
              <ServerActionForm action={completeSetupWithOAuth} className="space-y-4">
                ...
                <Button type="submit" className="h-9 w-full">Complete setup</Button>
              </ServerActionForm>
            )
          : (
              <>
                {localEnabled && (
                  <ServerActionForm action={setupLocalAdministrator} className="space-y-4">
                    ...
                    <Button type="submit" className="h-9 w-full">Create administrator</Button>
                  </ServerActionForm>
                )}
                {localEnabled && oauth.length > 0 && <Separator />}
                {oauth.length > 0 && (
                  <div className="flex flex-col gap-2"><OAuthProviderButtons providers={oauth} action="Continue" redirectTo="/setup" /></div>
                )}
